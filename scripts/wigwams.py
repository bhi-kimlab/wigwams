...
def merging(expr_df, overlap=0.3, meancorr=0.9, corrfilt=0.8, condspan = None, which_file='raw', legacy=False):
  ...
  while del_inds:
    ...
    for cat in categories:
      ...
      if (indmask == [] or len(indmask) == 0):  # bugfix
        continue
      #print indmask  # TODO for debugging (sometimes no-cluster selected, then program dies, so we need to fix this code)
      inds = inds[indmask]
...
def make_figure(modules, i, expr_df, cond_span, stand):
  ...
  for j in range(len(cond_span)):
    ...
    if (nrow < 2):  # bugfix
      print "error: too small row cannot draw"
      plt.close("all")
      return
...
def export(expr_df, deg_df, which_file, annot_file=None, hyper=None, stand=True):
  '''
  ...
  plot_counts = np.ceil(len(conditions)/9)+1
  print plot_counts
